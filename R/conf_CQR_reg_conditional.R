...
#' @export
conf_CQR_reg_conditional <- function(x, y, split, beta_grid, mtry_grid, ntree_grid, method = "efficient", alpha = 0.1){
  y_data <- as.vector(y)
  x_data <- as.matrix(x)
  N <- length(y_data)
  if(method == "efficient"){
  ...
  if(ret$cqr_method == "CQR"){
    ...
    pred_set_verify <- function(xx, yy){
      ...
      return(list(pmax(low, up) <= quant, abs(qhat_final[,2]-qhat_final[,1]) *(1 + quant),  qhat_final[,1]-quant*abs(qhat_final[,2] - qhat_final[,1]), qhat_final[,3]+quant*abs(qhat_final[,3] - qhat_final[,2]), qhat_final[,1], qhat_final[,3]  ))
    }  
    ret$pred_set <- pred_set_verify
    ...
    pred_set_verify <- function(xx, yy){
      ...
      return(list(pmax(low, up) <= quant, abs(qhat_final[,2]-qhat_final[,1]) ))
    return(list(pmax(low, up) <= quant, abs(qhat_final[,2]-qhat_final[,1]) *(1+2*quant), qhat_final[,1]-quant*abs(qhat_final[,2] - qhat_final[,1]), qhat_final[,2]+quant*abs(qhat_final[,2] - qhat_final[,1]), qhat_final[,1], qhat_final[,2] ))
    }  
    ret$pred_set <- pred_set_verify
  }
  ret$method <- method
